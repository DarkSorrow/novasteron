...
export const App = () => {
  ...
  const appReadySentRef = useRef(false);
    ...
    useEffect(() => {
        ...
        // We'll consider the app ready when:
        // 1. Either the model is loaded (if one was selected)
        // 2. Or no model is selected yet, but the UI is ready to accept input
        const isModelReady = !state.selectedModelFilePath || (
            state.model.loaded && 
            state.llama.loaded && 
    ...
    return <div className="app">
        <Stack direction="row" spacing={2}><div>row 1</div><div>row 2</div><div>row 3</div></Stack>
        <Header
