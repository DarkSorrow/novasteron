...
export function ModelResponseThought({ text, active, duration }: ModelResponseThoughtProps) {
  ...
  return (
    <div className={classNames('responseThought', active && 'active', isOpen && 'open')}>
      <button className="header" onClick={toggleIsOpen}>
        <span className="summary">
          <div className="title">{title}</div>
          <RightChevronIconSVG className="chevron" />
        </span>
        <MarkdownContent className={classNames('excerpt', isOpen && 'hide')} dir="auto" inline>
          {text.slice(-excerptLength)}
        </MarkdownContent>
      </button>
      <MessageMarkdown className={classNames('content', !isOpen && 'hide')} activeDot={active}>
        {text}
      </MessageMarkdown>
    </div>
  );
