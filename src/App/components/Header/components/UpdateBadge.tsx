...
export function UpdateBadge({ appVersion, canShowCurrentVersion }: UpdateBadgeProps) {
  ...
  useEffect(() => {
    if (appVersionIsBeta == null) return;
  ...
  return (
    <div className="updateBadge">
      ...
      {releasedVersionIsNewerThanCurrent && releaseLink != null && (
        <a target="_blank" href={releaseLink} className="newVersion">
  ...
  try {
    if (includePrerelease) {
      ...
      if (latestReleases.latestPrerelease != null && latestReleases.latestRelease != null) {
        if (
          semver.gt(latestReleases.latestPrerelease.version, latestReleases.latestRelease.version)
        )
          return {
    ...
    const releaseRes = await fetch(
      'https://api.github.com/repos/withcatai/node-llama-cpp/releases/latest'
    );
    const release: {
      tag_name: string;
...
async function getLatestPrereleaseAndRelease(): Promise<{
  ...
  try {
    const releasesRes = await fetch(
      'https://api.github.com/repos/withcatai/node-llama-cpp/releases?per_page=100'
    );
...
function normalizeTagName(tagName?: string) {
  ...
  if (tagName.toLowerCase().startsWith('v')) return tagName.slice('v'.length);
